...
/** D24: gráficos leves em SVG puro (sem dependência externa) para o Dashboard —
 * funil de conversão, ranking em barras, barras por mês e donut. */
...
export function BarrasMensais({ dados }: { dados: { rotulo: string; valor: number }[] }) {
  const max = Math.max(1, ...dados.map((d) => d.valor));
    ...
    <div style={{ display: "flex", alignItems: "flex-end", gap: 8, height: 140, padding: "4px 2px" }}>
      {dados.map((d, i) => (
        <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 4, height: "100%", justifyContent: "flex-end" }}>
          ...
          <div
            title={`${d.rotulo}: ${d.valor} processo(s)`}
            style={{
              width: "100%",
              maxWidth: 28,
...
export function Donut({ dados }: { dados: { rotulo: string; valor: number; cor: string }[] }) {
